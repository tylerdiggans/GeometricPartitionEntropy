...
figure;
h = histogram(x,Eh,'normalization','probability');
p = h.Values;
-dot(p,log2(p))
bar(h.Values)
...
set(gca,'fontsize',20) 
title({'Frequency Probability Distribution','(Histogram)'},'fontsize',14)

figure;
hcnts = Eq(2:end)-Eq(1:end-1);
-dot(hcnts,log2(hcnts))

bar(hcnts,'FaceColor','#EDB120')
xlabel('$k$','interpreter','latex')
ylabel('$q(k)$','interpreter','latex')
set(gca,'fontsize',20)  
title({'Geometric Probability Distribution','(Quantile)'},'fontsize',14)
