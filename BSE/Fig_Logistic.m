...
%x = sort([rand(500,1); 0.5*ones(500,1)]);
%pd = makedist('Beta','a',1,'b',5);
%x = sort(pd.random(1000,1));

quants = linspace(0,1,k+1);
V = [0 quantile(x,quants(1,2:end-1)) 1];
q = diff(V);
...
%xx = 0.01:0.01:0.99;
%yy = 0.5*ones(length(xx),1);
%yy(floor(length(xx)/2))=1;
%yy = betapdf(xx,1,5);
%figure('units','centimeters','position',[30,15,20,15]);
%area(xx,yy,0,'EdgeColor','none','FaceColor',"#0072BD",'FaceAlpha',0.25,'HandleVisibility','off'); hold on
%plot(xx,yy, 'Color', "#0072BD", 'LineWidth',4);
%title('Spike & Slab Distribution')
%title('Beta(1,5) Distribution')
%xlabel('x')
%5xticks([0 1])
